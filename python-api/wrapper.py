...
import ee
from gee_s1_ard import border_noise_correction as bnc
from gee_s1_ard import helper
from gee_s1_ard import speckle_filter as sf
from gee_s1_ard import terrain_flattening as trf
...
def s1_preproc(params):
    ...
    DEM = params["DEM"]
    TERRAIN_FLATTENING_ADDITIONAL_LAYOVER_SHADOW_BUFFER = params[
    ...
    if TERRAIN_FLATTENING_MODEL not in model_required:
        raise ValueError(
            "ERROR!!! Parameter TERRAIN_FLATTENING_MODEL not correctly defined"
        )
    ...
    if APPLY_SPECKLE_FILTERING:
        if SPECKLE_FILTER_FRAMEWORK == "MONO":
            s1_1 = ee.ImageCollection(
                sf.MonoTemporal_Filter(s1_1, SPECKLE_FILTER_KERNEL_SIZE, SPECKLE_FILTER)
            )
